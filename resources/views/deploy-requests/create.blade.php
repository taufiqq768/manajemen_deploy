                ...
                <form method="POST" action="{{ route('deploy-requests.store') }}" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <select id="kategori" name="kategori" required onchange="handleKategoriChange()"
                            ...
                            <option value="cr" {{ old('kategori') == 'cr' ? 'selected' : '' }}>CR (Change Request)</option>
                            <option value="enhancement" {{ old('kategori') == 'enhancement' ? 'selected' : '' }}>Enhancement</option>
                            <option value="bug_fixing" {{ old('kategori') == 'bug_fixing' ? 'selected' : '' }}>Bug Fixing</option>
                        </select>
                        @error('kategori')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                        ...
                        <div id="documents-container" class="space-y-4">
                            <div class="document-item p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-lg space-y-3">
                                <div class="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-2">
                                    <span class="text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider">Dokumen Pendukung #1</span>
                                </div>
                                <div>
