        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                @if(auth()->check())
                    ...
                    @if(auth()->user()->isAdmin() || auth()->user()->isProjectManager() || auth()->user()->isProgrammer())
                ...
                @if($user->isAdmin())
                    <div class="pt-4 pb-1 mt-4 border-t border-slate-200 dark:border-slate-800">
                        <p class="px-3 text-xs font-bold tracking-wider text-slate-400 uppercase">System</p>
                    </div>
                    ...
                    <a href="{{ route('api-docs') }}" target="_blank"
                        class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
