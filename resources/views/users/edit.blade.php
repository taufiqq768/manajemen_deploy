<x-layouts.app :title="'Edit Profil User'">
    ...
    <div class="max-w-2xl mx-auto">
        ...
        @endif
        @if($errors->any())
            <div class="mb-4 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-sm text-red-600 dark:text-red-400">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 sm:p-8">
                <h2 class="text-lg font-bold text-slate-900 dark:text-white mb-1">Edit Profil User</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Perbarui informasi dasar dan hak akses pengguna ini.</p>
                ...
                <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" id="phone_wa" name="phone_wa" value="{{ old('phone_wa', $user->phone_wa) }}"
                                placeholder="08..."
                        ...
                        <div>
                            ...
                            <select id="role" name="role" required class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('role') border-red-500 @enderror">
                                ...
                                <option value="governance" {{ old('role', $user->role) === 'governance' ? 'selected' : '' }}>Governance</option>
                                <option value="operational" {{ old('role', $user->role) === 'operational' ? 'selected' : '' }}>Operational</option>
                            </select>
            ...
            {{-- Reset Password Form --}}
            <div class="bg-white dark:bg-slate-900 border border-red-500/20 rounded-xl p-6 sm:p-8">
                <h2 class="text-lg font-bold text-red-600 dark:text-red-400 mb-1">Reset Password</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Paksa pembaruan password untuk pengguna ini.</p>

                <form method="POST" action="{{ route('users.reset-password', $user) }}" class="space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="password" id="password" name="password" required
                                class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-500 @error('password') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="password" id="password_confirmation" name="password_confirmation" required
                                class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-red-500">
                        </div>
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-2">
                        <button type="submit" onclick="return confirm('Apakah Anda yakin ingin mereset password untuk user ini?')"
                                class="px-5 py-2.5 bg-red-600 hover:bg-red-500 text-white text-sm font-medium rounded-lg transition-colors">
                            Reset Password
                        </button>
                    </div>
                </form>
