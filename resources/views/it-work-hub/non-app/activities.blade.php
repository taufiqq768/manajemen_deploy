<x-layouts.app>
    ...
    <div class="w-full px-4 2xl:px-8 mx-auto space-y-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('it-work-hub.non-app.show', $project->id) }}" class="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-slate-200 dark:border-slate-800">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <div id="tab-content-aktivitas" class="p-4">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button onclick="addRow('tbody-aktivitas')" class="px-3 py-1.5 bg-[#639922] hover:bg-[#3B6D11] text-white text-xs font-medium rounded-md shadow-sm transition-colors flex items-center gap-1">
                ...
                <div class="overflow-x-auto pb-4">
                    <table class="w-full text-left text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap">
                        ...
                        <tbody id="tbody-aktivitas" class="divide-y divide-slate-200 dark:divide-slate-800">
                            @php $activitiesList = $project->activities->sortBy('sort_order'); @endphp
                            ...
                            <tr class="empty-state">
                                <td class="px-3 py-8 text-center text-slate-400" colspan="11">Belum ada data Aktivitas. Klik "Tambah Baris" untuk menginput data.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        ...
        async function saveAllActivities() {
            ...
            try {
                const response = await fetch("{{ route('it-work-hub.non-app.activities.save', $project->id) }}", {
                    method: 'POST',
                    headers: {
