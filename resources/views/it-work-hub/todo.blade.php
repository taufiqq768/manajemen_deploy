<x-layouts.app>
    ...
    <div class="w-full px-4 2xl:px-8 mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
                <button onclick="saveAllTodos()" id="btn-save-all" class="px-5 py-2.5 bg-[#639922] hover:bg-[#3B6D11] text-white font-semibold rounded-lg shadow-sm transition-colors flex items-center gap-2">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-[#F1EFE8] dark:bg-slate-800/50">
                ...
                <button onclick="addRow('tbody-todos')" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-600 text-white text-xs font-medium rounded-md shadow-sm transition-colors flex items-center gap-1">
            ...
            <div class="overflow-x-auto pb-4">
                <table class="w-full text-left text-sm text-slate-600 dark:text-slate-400 whitespace-nowrap">
                    <thead class="bg-slate-200 dark:bg-slate-800 text-[10px] sm:text-xs uppercase font-semibold text-slate-700 dark:text-slate-300 border-b border-slate-300 dark:border-slate-700">
                        <tr>
                    ...
                    <tbody id="tbody-todos" class="divide-y divide-slate-200 dark:divide-slate-800">
                        ...
                        <tr data-id="{{ $todo->id }}" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                            ...
                            <td class="px-1 py-1"><input type="date" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-xs px-1 py-1.5 rounded input-date" value="{{ \Carbon\Carbon::parse($todo->date)->format('Y-m-d') }}"></td>
                            <td class="px-1 py-1">
                                <select class="input-user w-full" placeholder="Pilih PIC..." {{ !$isAdminOrPM ? 'disabled' : '' }}>
                                    @foreach($users as $u)
                                        <option value="{{ $u->id }}" {{ $todo->user_id == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td class="px-1 py-1"><input type="text" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-sm font-medium text-slate-800 dark:text-slate-200 px-2 py-1.5 rounded input-task" value="{{ $todo->task_name }}"></td>
                            <td class="px-1 py-1"><input type="date" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-xs px-1 py-1.5 rounded input-deadline" value="{{ \Carbon\Carbon::parse($todo->deadline)->format('Y-m-d') }}"></td>
                            <td class="px-1 py-1">
                                <select onchange="updateTodoStatusColor(this)" class="status-dropdown w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-[10px] font-bold px-2 py-1.5 rounded uppercase appearance-none cursor-pointer input-status">
                            ...
                            <td class="px-1 py-1"><input type="text" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-xs px-2 py-1.5 rounded input-notes" value="{{ $todo->notes }}" placeholder="Tambahkan catatan..."></td>
                        ...
                        <tr class="empty-state">
                            <td colspan="9" class="px-4 py-8 text-center text-slate-500">Belum ada tugas. Klik "Tambah Tugas" untuk membuat list baru.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <template id="row-template">
        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
            ...
            <td class="px-1 py-1"><input type="date" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-xs px-1 py-1.5 rounded input-date" value="{{ date('Y-m-d') }}"></td>
            <td class="px-1 py-1">
                <select class="input-user w-full" placeholder="Pilih PIC..." {{ !$isAdminOrPM ? 'disabled' : '' }}>
                    @foreach($users as $u)
            ...
            <td class="px-1 py-1"><input type="text" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-sm font-medium text-slate-800 dark:text-slate-200 px-2 py-1.5 rounded input-task" placeholder="Tugas baru..."></td>
            <td class="px-1 py-1"><input type="date" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-xs px-1 py-1.5 rounded input-deadline" value="{{ date('Y-m-d') }}"></td>
            <td class="px-1 py-1">
                <select onchange="updateTodoStatusColor(this)" class="status-dropdown w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-[10px] font-bold px-2 py-1.5 rounded uppercase appearance-none cursor-pointer input-status">
            ...
            <td class="px-1 py-1"><input type="text" class="w-full bg-transparent border-transparent hover:border-slate-300 focus:border-[#639922] focus:ring-1 focus:ring-[#639922] focus:bg-white text-xs px-2 py-1.5 rounded input-notes" placeholder="Tambahkan catatan..."></td>
            <td class="px-2 py-1 text-center opacity-0 group-hover:opacity-100 transition-opacity"><button type="button" class="text-slate-400 hover:text-red-500 transition-colors" title="Hapus Baris" onclick="removeRow(this)"><i class="ti ti-trash"></i></button></td>
        </tr>
    </template>
